						...
						<div class="panel-wrapper collapse in">
							<div class="panel-body">
								<div class="row " style="margin-bottom: 10px">
									<div class="col-md-1">
										<a href="/portal/internal/info tambah"><button class="btn btn-info" style="margin-bottom: 10px">Tambah</button></a> 
									</div>
								</div>
								<div class="row">
									<div class="table-responsive">
										<table class="myTable table table-hover table-bordered">
											...
											<tbody>
												...
												<tr>
													<td>{{ $key + 1 }}</td>
													<td>{{ date('d/m/Y H:i:s', strtotime(str_replace('/', '-', $info['tgl']))) }}</td>
													<td>{{ $info['info_judul'] }}</td>
													<td>{{ date('d/m/Y', strtotime(str_replace('/', '-', $info['tgl_mulai']))) }} - {{ date('d/m/Y', strtotime(str_replace('/', '-', $info['tgl_akhir']))) }}</td>
													<td>
														<a target="_blank" href="{{ config('app.openfileinfo') }}/{{ $info['ids'] }}/{{ $info['info_file'] }}"><i class="fa fa-download"></i> [Unduh Disini] </a>
													</td>
													<td>
														@if($info['info_tampil'] == 1)
													...
													<td class="col-md-1">
														<form method="POST" action="/portal/internal/info ubah">
															@csrf
															...
															<button type="button" class="btn btn-danger btn-outline btn-circle m-r-5 btn-delete" data-toggle="modal" data-target="#modal-delete" data-ids="{{ $info['ids'] }}"><i class="fa fa-trash"></i></button>
														</form>
													</td>
												</tr>
												@endforeach
			...
			<div id="modal-delete" class="modal fade" role="dialog">
				<div class="modal-dialog">
					<div class="modal-content">
						<form method="POST" action="/portal/internal/form/hapusinfo" class="form-horizontal">
						@csrf
							<div class="modal-header">
								<h4 class="modal-title"><b>Hapus Info Kepegawaian</b></h4>
							</div>
	...
	<script>
		$(function () {
			...
			$('.btn-delete').on('click', function () {
				...
				$("#label_delete").append('Apakah anda yakin ingin menghapus info tersebut?');
