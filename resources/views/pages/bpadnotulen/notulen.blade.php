						...
						<div class="panel-wrapper collapse in">
							<div class="panel-body">
								<div class="row" style="margin-bottom: 10px">
									<form method="GET" action="/portal/notulen/notulen">
										<div class="row">
											<div class=" col-md-2">
					                          	...
					                          	<?php foreach ($units as $key => $unit) { ?>
					                            	<option value="{{ $unit['kd_unit'] }}" 
					                              	<?php 
					                                	...
					                                	if ($idunit == $unit['kd_unit'] || strpos($unit['kd_unit'], $idunit)) {
						                                 	echo "selected";
						                                }
					                              	?>
					                            	>[{{ $unit['kd_unit'] }}] - {{ ($unit['kd_unit'] == '01' ? 'SEMUA' : $new_nm)   }}</option>
					                          	<?php } ?>
					                        	</select>
				                      		</div>
										</div>
									...
									<div class="table-responsive">
										<table class="myTable table table-hover table-striped">
											...
											<tbody>
												...
												<tr>
													<td>{{ $key + 1 }}</td>
													<td>{{ strtoupper($notulen['nm_emp']) }}</td>
													<td>{{ date('d-M-Y', strtotime($notulen['not_tanggal'])) }}<br>
														<span class="text-muted">
															{{ date('H:i', strtotime($notulen['not_mulai'])) }} - {{ date('H:i', strtotime($notulen['not_selesai'])) }}
														</span>
													</td>
													<td>{{ $notulen['not_acara'] }}</td>
														...
														<td class="col-md-1">
															<form method="POST" action="/portal/internal/notulen ubah">
																...
																@if($access['zupd'] == 'y')
																	...
																	<input type="hidden" name="ids" value="{{ $notulen['ids'] }}">
																	<button type="submit" class="btn btn-info btn-outline btn-circle m-r-5 btn-update"><i class="ti-pencil-alt"></i></button>
																	
																@endif
																@if($access['zdel'] == 'y')
																	<button type="button" class="btn btn-danger btn-outline btn-circle m-r-5 btn-delete" data-toggle="modal" data-target="#modal-delete" data-ids="{{ $notulen['ids'] }}"  ><i class="fa fa-trash"></i></button>
																@endif
															</form>
														</td>
													@endif
