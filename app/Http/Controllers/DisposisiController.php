		...
		if (isset($request->btnKirim)) {
			...
			if (isset($request->jabatans)) {
				for ($i=0; $i < count($request->jabatans); $i++) { 
					...
					if (!(in_array($request->jabatans[$i], $arrjabatan))) {
						array_push($arrjabatan, $request->jabatans[$i]);
			...
			if (isset($request->stafs)) {
				for ($i=0; $i < count($request->stafs); $i++) { 
					...
					if (!(in_array($request->stafs[$i], $arrstaf))) {
						array_push($arrstaf, $request->stafs[$i]);
